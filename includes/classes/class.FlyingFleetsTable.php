...
class FlyingFleetsTable
{
	...
	private function BuildFleetEventTable($fleetRow, $FleetState)
	{
		...
		// Determine if this fleet is hostile (incoming attack/destroy/spy/missile from another player)
		$isOwner	= ($fleetRow['fleet_owner'] == $this->userId);
		$hostileMissions = array(1, 2, 6, 9, 10); // attack, acs, spy, destroy, missile
		$isHostile	= (!$isOwner && in_array((int) $fleetRow['fleet_mission'], $hostileMissions) && $FleetState == 0);

		return array(
			'text'			=> $EventString,
			'returntime'	=> $Time,
			'resttime'		=> $Rest,
			'isHostile'		=> $isHostile ? 1 : 0,
			'mission'		=> (int) $fleetRow['fleet_mission'],
			'isOwn'			=> $isOwner ? 1 : 0,
		);
	}
