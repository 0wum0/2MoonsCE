 ...
 *
 * Supported tags:
 *   [b] [i] [u] [s] [center] [right]
 *   [url] [url=...] [img] [img=WxH]
 *   [quote] [quote=Name]
 *   [code] [code=lang]
 *   [list] [list=1] [list=a]  with [*] items
 ...
 *
 * Security:
 *   - Input is always HTML-escaped before parsing (no raw HTML from users).
 *   - [url]/[img] hrefs are validated; javascript:/data: schemes are blocked.
 *   - nl2br is applied only outside block-level elements (lists, tables, code,
 *     quotes) so <br> never appears inside <li>, <td>, <pre> etc.
 *   - [code] content is NOT double-escaped: the outer htmlspecialchars pass has
 *     already escaped it; parseCode works on the already-escaped string.
 *   - html_entity_decode is run first so stored &amp; / &quot; etc. do not
 *     break BBCode tag matching.
 */
class BBCode
{
    private Database $db;
