...
class ShowFleetStep2Page extends AbstractGamePage
{
	...
	public function show()
	{
		...
		$this->assign(array(
			...
			'maxspeed'						=> (int) $MaxFleetSpeed,
			'duration'						=> (int) $duration,
			'arrivalTime'					=> _date($LNG['php_tdformat'], TIMESTAMP + (int) $duration, $USER['timezone']),
			'returnTime'					=> _date($LNG['php_tdformat'], TIMESTAMP + ((int) $duration * 2), $USER['timezone']),
			'fleetroom'						=> (int) $_SESSION['fleet'][$token]['fleetRoom'],
