...
class ShowFleetTablePage extends AbstractGamePage
{
	...
	public function show()
	{
        ...
        // FIX: Clean up expired returned fleets before counting active slots
        // This prevents "stuck fleet slots" when cron hasn't processed returned fleets
        $sqlExpired = "SELECT fleet_id FROM %%FLEETS%% 
            WHERE fleet_owner = :userID 
            AND fleet_mess = 1 
            AND fleet_end_time <= :now;";
        $expiredFleets = $db->select($sqlExpired, array(
            ':userID' => $USER['id'],
            ':now'    => TIMESTAMP
        ));
        if (!empty($expiredFleets)) {
            ...
            foreach ($expiredFleets as $expFleet) {
                $sqlF = "SELECT * FROM %%FLEETS%% WHERE fleet_id = :fid;";
                $fRow = $db->selectSingle($sqlF, array(':fid' => $expFleet['fleet_id']));
                if (!empty($fRow) && isset(FlyingFleetHandler::$missionObjPattern[$fRow['fleet_mission']])) {
                    $mName = FlyingFleetHandler::$missionObjPattern[$fRow['fleet_mission']];
                    $mPath = 'includes/classes/missions/' . $mName . '.class.php';
