...
function ShowFlyingFleetPage(): void
{
    ...
    // Handle fleet lock/unlock toggle
    $id = HTTP::_GP('id', 0);
    if (!empty($id)) {
        $lock    = HTTP::_GP('lock', 0);
        $lockVal = ($lock == 0) ? 'NULL' : "'ADM_LOCK'";

        $db->update(
            "UPDATE %%FLEETS%%
             SET `fleet_busy` = :lock
             WHERE `fleet_id` = :id AND `fleet_universe` = :uni;",
            [':lock' => $lock, ':id' => (int) $id, ':uni' => (int) $uni]
        );

        // fleets_event.lock is either NULL or the string 'ADM_LOCK' — use raw query
        // since PDO cannot bind SQL NULL via a named param in UPDATE SET expressions
        // without extra logic; keep as nativeQuery for this edge case.
        $db->nativeQuery(
            "UPDATE %%FLEETS_EVENT%%
             SET `lock` = {$lockVal}
             WHERE `fleetID` = " . (int) $id . ";"
        );
    }
    ...
    $fleetRows = $db->select(
        "SELECT
             ...
             acs.name              AS acsName
         FROM %%FLEETS%% fleet
         LEFT JOIN %%FLEETS_EVENT%% event   ON event.fleetID    = fleet.fleet_id
         LEFT JOIN %%PLANETS%%      pstart  ON pstart.id        = fleet.fleet_start_id
         LEFT JOIN %%PLANETS%%      ptarget ON ptarget.id       = fleet.fleet_end_id
         LEFT JOIN %%USERS%%        ustart  ON ustart.id        = fleet.fleet_owner
         LEFT JOIN %%USERS%%        utarget ON utarget.id       = fleet.fleet_target_owner
         LEFT JOIN %%AKS%%          acs     ON acs.id           = fleet.fleet_group
         WHERE fleet.fleet_universe = :uni
         GROUP BY fleet.fleet_id
         ORDER BY fleet.fleet_id ASC;",
        [':uni' => (int) $uni]
    );
    ...
    foreach ($fleetRows as $fleetRow) {
        ...
        $fleetList[] = [
            ...
            'staytime'           => $fleetRow['fleet_start_time'] !== $fleetRow['fleet_end_stay']
                                        ? _date($LNG['php_tdformat'], $fleetRow['fleet_end_stay'], $USER['timezone'])
                                        : 0,
