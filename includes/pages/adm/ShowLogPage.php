...
class ShowLogPage extends AbstractAdminPage
{
	...
	{
		parent::__construct('ShowLogPage');
	}
	...
	protected function run(): void
	{
		$table = HTTP::_GP('type', '');
		switch ($table) {
			case 'planet':   $this->showPlanetsList();   break;
			case 'player':   $this->showPlayersList();   break;
			case 'settings': $this->showSettingsList();  break;
			case 'present':  $this->showPresent();       break;
			case 'detail':   $this->showDetail();        break;
			default:         $this->showOverview();      break;
		}
	}
	...
	private function showDetail(): void
	{
		...
		$result = $db->selectSingle(
			"SELECT l.*, u_a.username as admin_username
			 FROM %%LOG%% as l
		...
		foreach ($conf_before as $key => $val) {
			...
			if (isset($LNG['tech'][$key]))       $Element = $LNG['tech'][$key];
			elseif (isset($LNG['se_' . $key]))   $Element = $LNG['se_' . $key];
			elseif (isset($LNG[$key]))            $Element = $LNG[$key];
			elseif (isset($Wrapper[$key]))        $Element = $Wrapper[$key];
			else                                  $Element = $key;
	...
	private function showSettingsList(): void
	{
		...
		$db   = Database::get();
		$rows = $db->select(
			"SELECT l.id, l.admin, l.time, l.universe, l.target,
			        u_a.username as admin_username
			 FROM %%LOG%% as l
			 LEFT JOIN %%USERS%% as u_a ON u_a.id = l.admin
			 WHERE mode = 3 ORDER BY id DESC;"
		);
		...
		foreach ($rows as $LogRow) {
			$LogArray[] = [
				...
				'target'     => $targetkey[$LogRow['target']] ?? $LogRow['target'],
	...
	private function showPlanetsList(): void
	{
		...
		$db   = Database::get();
		$rows = $db->select(
			"SELECT DISTINCT l.id, l.admin, l.target, l.time, l.universe,
			        u_t.username as target_username,
			        p.galaxy as target_galaxy, p.system as target_system, p.planet as target_planet,
			        u_a.username as admin_username
			 FROM %%LOG%% as l
			 LEFT JOIN %%USERS%%   as u_a ON u_a.id = l.admin
			 LEFT JOIN %%PLANETS%% as p   ON p.id   = l.target
			 LEFT JOIN %%USERS%%   as u_t ON u_t.id = p.id_owner
	...
	private function showPresent(): void
	{
		...
		$rows = $db->select(
			"SELECT DISTINCT l.id, l.admin, l.target, l.time, l.universe,
			 ...
			 FROM %%LOG%% as l
			 LEFT JOIN %%USERS%% as u_a ON u_a.id = l.admin
			 WHERE mode = 4 ORDER BY l.id DESC;"
		);
